...
#>
param (
	[string]$TheProjectLocation,
	#e.g. "$env:FlywayWorkPath\<database>\branches\develop"

	[string]$PathToSecretsFile,
	#To run Flyway info e.g. $env:userProfile\<database>_SQLServer_develop_<server>1.toml"

	[array]$MigrationLocationList,
	#@('<database>\Branches\Develop\Migrations')
...
{
	Write-error @"
 could not proceed, in directory '$PWD', using credentials at $currentCredentialsPath
"@ -ErrorAction Stop
}

<#We can now use the output of Flyway's info command to get the list of files that the 
config provides #>
...
<# Now we have a complete list of every script  referenced in a migration run, with
 ...
 within the project 
#>
$visited = @{ } <# we must start with a NULL list of files we've visited - or files
we don't want visited. we need a function to do this as it is recursive so we grab the
current version of the file it is in #>
...
$ScriptFilesUsedByFlyway | foreach {
	...
	else
	{
		...
		{ write-host "File $($_) is not Source-controlled within the project" }
		elseif ($Actions.Count -eq 1)
		{ write-host  "File $($($Actions[0].File)) -  $($Actions[0].message) at  $($Actions[0].Date)" }
		else
		{
			write-host  "File $($Actions[0].File) - $(($Actions | foreach{
						"`n    $($_.message) at  $($_.Date)"
					}) -join ",")"
		}
	}
}
