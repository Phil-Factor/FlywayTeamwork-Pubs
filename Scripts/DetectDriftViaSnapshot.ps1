...
$DiffParams = @(
	"-diff.target=snapshot:$CurrentArtefactLocation\Snapshot.json", <#
...
if ($Differences.individualResults[0].differences.Count -gt 0)
{
	$Changes = $Differences.individualResults[0].differences | foreach {
		...
		$Changes | ConvertTo-Csv > "$CurrentReportLocation\changes.csv"
	}
}
else
{
	Write-Host "no changes were found in $Project $Branch $Version"
}