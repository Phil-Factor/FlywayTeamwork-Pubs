...
# it is worth grabbing
 ...
 $DBDetails = @{
	...
	'ProjectFolder' = $FlywayContent.'flyway.locations'; #parent the scripts directory
...
$FlywayValues = (gci env:FP__*) + (gci env:Flyway_*) | foreach{
	if ($_.name -imatch '(FP__flyway_(?<Variable>[\w]*)__|FP__(?<Variable>[\w]*)__|FLYWAY_(?<Variable>[\w]*))')
	{
		...
		{
			$FlywayName = $FlywayName.ToLower().Split() | foreach -Begin { $ii = $false; $res = '' }{ $res += if ($ii) { (Get-Culture).TextInfo.ToTitleCase($_) }
				else { $_; $ii = $true }; } -End { $res }
		}
		@{ $FlywayName = $_.Value }
	}
}
#add in any missing details
$FlywayValues|foreach{if (!($dbDetails.Contains("$($_.Keys)"))){$dbDetails+=$_}}
