...
DECLARE @auditTable TABLE
  (Action VARCHAR(40) NOT NULL,
   ToWhat VARCHAR(300) NOT NULL,
   ItsName VARCHAR(128) NOT NULL,
   TheDescription NVARCHAR(3750) NOT NULL);
...
INSERT INTO @TableSourceToDocument
  (TableObjectName, ObjectType, TheDescription, TheColumns)
...
WHILE @ii <= @iiMax
  BEGIN
    SELECT @Schemaname = Object_Schema_Name (Object_Id (TableObjectName)),
           ...
           @TypeCode =
             CASE ObjectType WHEN 'clr table valued function' THEN 'FT'
...
-- SQL Prompt formatting off
		...
		BEGIN
			SELECT @action =
              N'Added description to the '+ Coalesce(@ObjectType,'unknown') + N' ' + Coalesce(@Schemaname,'unknown') + '.'+ Coalesce(@TableObjectName,'unknown');
            PRINT @action;
            INSERT INTO @auditTable (Action, ToWhat, ItsName, TheDescription)
            VALUES
              ('Added', @ObjectType,
               @Schemaname + '.' + @TableObjectName,
               @TheDescription);
        ...
        ELSE
 			BEGIN
			SELECT @action =
              N'updated description to the '+ Coalesce(@ObjectType,'unknown') + N' ' + Coalesce(@Schemaname,'unknown') + '.'+ Coalesce(@TableObjectName,'unknown');
            PRINT @action;
            INSERT INTO @auditTable (Action, ToWhat, ItsName, TheDescription)
            VALUES
              ('Added', @ObjectType,
 ...
 -- SQL Prompt formatting on 
        ...
        INSERT INTO @ColumnToDocument
          (TheDescription, Level0Name, ObjectType, TableObjectName,
           Level2Name)
...
-- SQL Prompt formatting on
          BEGIN
            SELECT @action =
              N'Added description to the ' + @Level2Name + N' column of '
              + @Level0Name + N'.' + @ObjectType + N' ' + @TableObjectName;
            PRINT @action;
            INSERT INTO @auditTable (Action, ToWhat, ItsName, TheDescription)
            VALUES
              ('Added', @ObjectType + ' column',
               @Level0Name + '.' + @TableObjectName + '.' + @Level2Name,
               @TheDescription);
            EXEC sys.sp_addextendedproperty 
			    @name = N'MS_Description',
                @value = @TheDescription,
        ...
        ELSE --update it 
          BEGIN
            PRINT 'Updated description to the ' + @Level2Name + ' column of '
                  + @Level0Name + '.' + @ObjectType + ' ' + @TableObjectName;
            INSERT INTO @auditTable (Action, ToWhat, ItsName, TheDescription)
            VALUES
              ('Updated', @ObjectType + ' column',
               @Level0Name + '.' + @TableObjectName + '.' + @Level2Name,
               @TheDescription);
            EXEC sys.sp_updateextendedproperty
				@name = N'MS_Description',
    ...
    SELECT @ii = @ii + 1; -- on to the next one
  END;
SELECT Action, ToWhat, ItsName, TheDescription FROM @auditTable; --tell the world what you've done
GO
