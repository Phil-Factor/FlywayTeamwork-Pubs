...
(
    ...
    WHERE object_id = OBJECT_ID(N'[people].[FKWordOccurenceWord]', 'F')
...
(
    SELECT 1
    FROM sys.objects
    WHERE object_id = OBJECT_ID(N'[people].[PKWordOcurrence]', 'PK')
          AND parent_object_id = OBJECT_ID(N'[people].[WordOccurence]', 'U')
)
...
(
    SELECT 1
    FROM sys.objects
    WHERE object_id = OBJECT_ID(N'[people].[PKWord]', 'PK')
...
(
    ...
    WHERE name = N'frequency'
          ...
          AND default_object_id = OBJECT_ID(N'[people].[DF__Word__frequency__56E8E7AB]', 'D')
)
...
(
    SELECT 1
    FROM sys.objects
    WHERE object_id = OBJECT_ID(N'[dbo].[FindString]')
          AND
          (
              type = 'IF'
              OR type = 'FN'
...
(
    SELECT 1
    FROM sys.objects
    WHERE object_id = OBJECT_ID(N'[dbo].[FindWords]')
          AND
          (
              type = 'IF'
              OR type = 'FN'
...
(
    SELECT 1
    FROM sys.objects
    WHERE object_id = OBJECT_ID(N'[dbo].[IterativeWordChop]')
          AND
          (
              type = 'IF'
              OR type = 'FN'
...
EXEC sp_updateextendedproperty N'Database_Info',
                               N'[{"Name":"Pubs","Version":"1.1.1","Description":"The Pubs (publishing) Database supports a fictitious publisher.","Modified":"2022-02-04T11:07:40.790","by":"PhilFactor"}]',
                               NULL,
                               NULL,
                               NULL,
                               NULL,
