...
-- First, create the table without the calculated column
CREATE TABLE People.Person 
(
  person_ID INT GENERATED BY DEFAULT AS IDENTITY CONSTRAINT PersonIDPK PRIMARY KEY,
  ...
  LegacyIdentifier NATIONAL CHARACTER VARYING (30) NULL,
  ModifiedDate TIMESTAMPTZ NOT NULL CONSTRAINT PersonModifiedDateD DEFAULT now()
);

-- Then, add the calculated column using ALTER TABLE
ALTER TABLE People.Person
ADD fullName NATIONAL CHARACTER VARYING (80) GENERATED ALWAYS AS
  (COALESCE(Title || ' ', '') ||
   FirstName ||
   COALESCE(' ' || MiddleName, '') ||
