...
respectively (no partial payments) */
  (id INTEGER PRIMARY KEY,
   ...
   description text NOT NULL, --Receipt from organisation, Receipt from individual
...
CREATE TABLE accounting.Received_Moneys
  ...
  (id INTEGER PRIMARY KEY,
   tran_date DATE NOT NULL,
   description text,             --eg Interest from bank, Proceeds from shares issue
   reference text,
   total DECIMAL(20, 2) NOT NULL,
...
CREATE TABLE accounting.Spent_Moneys /* may have an optional Supplier*/
  (id INTEGER PRIMARY KEY,
   tran_date DATE NOT NULL,
   description text,             --Payment of interest or Payment of taxes
   reference text,
   total DECIMAL(20, 2) NOT NULL,
   supplier_id INTEGER,
   Chart_of_Accounts_id INTEGER NOT NULL, -- automatically Bank
...
AS
  WITH itrans
  AS (
     SELECT Cast  (i.id as VARCHAR(10)) AS tran_id, i.tran_date,
...
AS
  WITH btrans
  AS (
     SELECT Cast  (b.id as VARCHAR(10)) AS tran_id, b.tran_date,
...
AS
  SELECT 'RM' || Cast  (rm.id as VARCHAR(10)) AS tran_id, tran_date,
...
AS
  SELECT 'SM' || Cast  (sm.id as VARCHAR(10)) AS tran_id, tran_date,
         Chart_of_Accounts_id,
         'Business Bank Account' AS Chart_of_Accounts_name, total,
