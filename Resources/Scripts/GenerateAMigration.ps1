<#	
	...
	.DESCRIPTION
		...
		A hierarchical file structure is required for proper execution so that the script can know where
        the resources are. Otherwise you'd need a mapping of some sort.


    .EXAMPLE pwsh -File  pwsh -File  "<pathTo>\FlywayTeamwork\Resources\DoACommand.ps1" -command "undo" `
                   -parameters "-target" -filename "<Current File Edited>"-Server "<server>" -ServerType   "<serverType>"
#>
param (
	$command = 'Generate', # The Flyway command(s) to execute (default: 'info')
	...
	$Server = 'Philf01', # The database server name (wildcard '*' if using a single server)
